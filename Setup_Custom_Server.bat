...
IF %ERRORLEVEL% NEQ 0 (
    echo curl is not installed. This script failed to run. Please check the manual install tutorial on https://au.niko233.top
    echo curl在你的电脑上没有安装，本脚本无法执行。请参考 https://au.niko233.top 上的手动安装方式
    pause
    exit /b
)
...
IF %ERRORLEVEL% NEQ 0 (
    ...
    echo Or you can try the following manual download method on https://au.niko233.top.
    echo 下载配置文件失败，请检查你的网络连接状况并重试。部分地区移动和有福地区存在屏蔽，只能手动下载。
    echo 你可以尝试手动安装，参考教程 https://au.niko233.top
    pause
    exit /b
)
